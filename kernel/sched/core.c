...
atomic_t bfq_mode = ATOMIC_INIT(BFQ_MODE_UNKNOWN);

enum bfq_mode_state bfq_get_mode(void)
{
	return atomic_read(&bfq_mode);
}
EXPORT_SYMBOL_GPL(bfq_get_mode);

void bfq_set_mode(enum bfq_mode_state mode)
{
	atomic_set(&bfq_mode, mode);
}
EXPORT_SYMBOL_GPL(bfq_set_mode);

atomic_t bfq_scheduling_burst = ATOMIC_INIT(0);

int bfq_get_scheduling_burst(void)
{
	return atomic_read(&bfq_scheduling_burst);
}
EXPORT_SYMBOL_GPL(bfq_get_scheduling_burst);

int bfq_inc_scheduling_burst(void)
{
	return atomic_inc_return(&bfq_scheduling_burst);
}
EXPORT_SYMBOL_GPL(bfq_inc_scheduling_burst);

int bfq_dec_scheduling_burst(void)
{
	int old, new;

	do {
		old = atomic_read(&bfq_scheduling_burst);
		if (old <= 0)
			return 0;
		new = old - 1;
	} while (atomic_cmpxchg(&bfq_scheduling_burst, old, new) != old);

	return new;
}
EXPORT_SYMBOL_GPL(bfq_dec_scheduling_burst);

/**
 * ioprio_override_revoke - clear burst I/O override with correct BFQ accounting.
 *
 * Must be used when clearing ioprio_override other than in context_switch(), so
 * bfq_scheduling_burst stays consistent when the task is currently running.
 */
void ioprio_override_revoke(struct task_struct *p)
{
	struct rq_flags rf;
	struct rq *rq;
	bool dec_burst = false;

	rq = task_rq_lock(p, &rf);
	if (READ_ONCE(p->ioprio_override)) {
		WRITE_ONCE(p->ioprio_override, false);
...
{
	if (!prev->ioprio_override && next->ioprio_override) {
		bfq_inc_scheduling_burst();
	} else if (prev->ioprio_override && !next->ioprio_override) {
		bfq_dec_scheduling_burst();
	}
