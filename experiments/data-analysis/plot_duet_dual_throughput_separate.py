...
# -*- coding: utf-8 -*-
"""
以块大小为横坐标，不同duet_dual阈值配置为柱状图的sustained throughput分析
数据来源于高低阈值综合实验.xlsx
绘制四个子图，分别绘制2、4、8、16个Background数量的throughput
子图1：绘制2个Background数量的throughput，横坐标为块大小，纵坐标为throughput，颜色为阈值标签，点型为块大小
子图2：绘制4个Background数量的throughput，横坐标为块大小，纵坐标为throughput，颜色为阈值标签，点型为块大小
子图3：绘制8个Background数量的throughput，横坐标为块大小，纵坐标为throughput，颜色为阈值标签，点型为块大小
子图4：绘制16个Background数量的throughput，横坐标为块大小，纵坐标为throughput，颜色为阈值标签，点型为块大小
...
def analyze_duet_dual_throughput_data(file_path):
    ...
    for config in sorted(configs):
        ...
        # 提取高低阈值，并添加到dual_threshold_mapping字典中
...
def plot_single_dual_throughput_chart(plot_data, count, output_dir):
    """
    绘制单个Background数量的duet_dual throughput图
    """
    # 创建单个图 - 增加宽度
    ...
    # 保存图片为figures-main文件
    plt.savefig(f'{output_dir}/duet_dual_throughput_background_{count}.png', dpi=300)
    plt.savefig(f'{output_dir}/duet_dual_throughput_background_{count}.pdf')
...
def main():
    ...
    print(f"\n正在生成四张独立的duet_dual throughput图片到 {output_dir} 文件夹...")
