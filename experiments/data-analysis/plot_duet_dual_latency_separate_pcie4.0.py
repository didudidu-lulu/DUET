...
# -*- coding: utf-8 -*-
"""
PCIe 4.0 设备：以块大小为横坐标，不同 duet_dual 阈值配置为折线的 latency 分析。
...
def plot_single_dual_latency_chart(plot_data, count, output_dir):
    ...
    for j, threshold_label in enumerate(threshold_labels):
        ...
        ax.errorbar(
            ...
            marker=markers[j % len(markers)], linewidth=3.0, markersize=10,
    ...
    ax.set_xticks(range(len(all_block_sizes)))
    ...
    prefix = f'{output_dir}/duet_dual_latency_pcie4.0_background_{count}'
...
def main():
    ...
    print("正在分析 PCIe 4.0 duet_dual latency 数据...")
    df, dual_threshold_mapping = analyze_duet_dual_data(DATA_FILE)

    print("\n正在准备 PCIe 4.0 duet_dual latency 绘图数据...")
    ...
    count_list = [2, 4, 8, 16]
    print(f"\n正在生成 PCIe 4.0 latency 图片到 {OUTPUT_DIR} 文件夹...")
