...
bg_bs_sizes=("4k")           # 背景流的块大小
bg_iodepths=("1" "4" "16" "64" "256" "1024")              # 背景流的队列深度
bg_counts=("4")                 # 背景流数量
repeat_count=3                          # 每组参数重复测试次数
local_mode=("none" "bfq")  # 测试模式，包括无调度器、BFQ调度器和BFQ高优先级调度器
...
run_test() {
    ...
    # 优化后的测试配置文件名（包含thinktime）
    local test_name="burst_rw-${burst_rw}_burst_bs-${burst_bs}_burst_iodepth-${burst_iodepth}_burst_thinktime-${burst_thinktime}_bg_rw-${bg_rw}_bg_bs-${bg_bs}_bg_iodepth-${bg_iodepth}_bg_count-${bg_count}_iter-${iteration}"
    ...
    # 修改burst_io的thinktime参数（使用传入的值）
    sed -i "s/thinktime=1s/thinktime=${burst_thinktime}/" "$test_config"
...
"
    ...
    log_message "开始测试: $test_name (thinktime: $burst_thinktime)"
    ...
    if [ "$mode" = "bfq_limit" ]; then
        ./bdev_set_bytes /dev/nvme0n1 1 6291456 6291456
    ...
    log_message "完成测试: $test_name (thinktime: $burst_thinktime)"
}
...
for mode in "${local_mode[@]}"; do
    if [ "$mode" = "none" ]; then
        ...
        ./bdev_set_bytes /dev/nvme0n1 0 131072 131072
        scheduler_tag="bfq"
    ...
    for bg_count in "${bg_counts[@]}"; do  # bg_count移到第二层
        for burst_thinktime in "${burst_thinktimes[@]}"; do  # burst_thinktime移到第三层
            results_dir="test_results_${scheduler_tag}_bgcount${bg_count}_thinktime${burst_thinktime}_$(date +%Y%m%d_%H%M%S)"
            mkdir -p "$results_dir"
                            ...
                            for burst_iodepth in "${burst_iodepths[@]}"; do
                                for burst_bs in "${burst_bs_sizes[@]}"; do
                                    for ((iter=1; iter<=repeat_count; iter++)); do
                                        run_test "$bg_rw" "$bg_bs" "$bg_iodepth" "$bg_count" "$burst_rw" "$burst_iodepth" "$burst_bs" "$burst_thinktime" "$iter" "$mode"
                                        sleep 5  # 测试间隔
                                    done
                                done
                            done
