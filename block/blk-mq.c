...
static inline void blk_mq_account_free_request(struct request *rq)
{
	struct request_queue *q = rq->q;

	if (rq->rq_flags & RQF_STARTED) {
		atomic_long_sub((long)rq->request_size_bytes, &q->io_bytes);
	}
	if (q->io_bytes_enable && rq->ioprio_override &&
	    q->elevator && q->elevator->type &&
	    !strcmp(q->elevator->type->elevator_name, "bfq")) {
		atomic_dec(&q->burst_ios);
		rq->ioprio_override = 0;
	}
...
void blk_mq_free_request(struct request *rq)
{
	...
	blk_mq_account_free_request(rq);
