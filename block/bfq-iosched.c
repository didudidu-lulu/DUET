...
static void bfq_update_burst_mode(struct request_queue *q);
				...
				struct request *next)
{
	...
	if (next->ioprio_override) {
		if (bfqq->bfqd->queue->io_bytes_enable)
			atomic_dec(&bfqq->bfqd->queue->burst_ios);
		if (!rq->ioprio_override) {
			rq->ioprio_override = true;
			if (bfqq->bfqd->queue->io_bytes_enable)
				atomic_inc(&bfqq->bfqd->queue->burst_ios);
		}
		bfq_update_burst_mode(bfqq->bfqd->queue);
	}
...
static unsigned int bfq_current_io_bytes_threshold(struct request_queue *q)
{
	unsigned int threshold = UINT_MAX;
	int burst_sum;
	int sched_burst;

	if (!q->io_bytes_enable)
		return threshold;

	sched_burst = bfq_get_scheduling_burst();
	if (sched_burst < 0)
		sched_burst = 0;
	burst_sum = atomic_read(&q->burst_ios) + sched_burst;
	threshold = burst_sum > 0 ? q->io_low_bytes : q->io_high_bytes;
	atomic_long_set(&q->io_bytes_threshold, threshold);
...
static void bfq_update_burst_mode(struct request_queue *q)
{
	enum bfq_mode_state old_mode, new_mode;

	if (!q->io_bytes_enable)
		return;

	if (atomic_read(&q->burst_ios) + bfq_get_scheduling_burst() > 0)
		new_mode = BFQ_MODE_BURST;
	else
		new_mode = BFQ_MODE_SUSTAINED;

	old_mode = bfq_get_mode();
	if (old_mode != new_mode) {
		pr_info_ratelimited(
			"bfq_mode_switch dev=%s jiffies=%lu mono_ns=%llu old_mode=%d new_mode=%d io_bytes=%ld threshold=%ld burst_ios=%d sched_burst=%d pid=%d comm=%s\n",
			q->disk ? q->disk->disk_name : "unknown", jiffies,
			ktime_get_ns(), old_mode, new_mode,
			atomic_long_read(&q->io_bytes),
			atomic_long_read(&q->io_bytes_threshold),
			atomic_read(&q->burst_ios), bfq_get_scheduling_burst(),
			current->pid, current->comm);
	}

	bfq_set_mode(new_mode);
}
...
static bool bfq_has_work(struct blk_mq_hw_ctx *hctx)
{
	...
	unsigned int threshold = bfq_current_io_bytes_threshold(q);

	/*
	 * Avoiding lock: a race on bfqd->queued should cause at
	 * most a call to dispatch for nothing
	 */
	if (q->io_bytes_enable && atomic_long_read(&q->io_bytes) > threshold &&
	    atomic_read(&q->burst_ios) <= 0)
		return false;
...
static struct request *__bfq_dispatch_request(struct blk_mq_hw_ctx *hctx)
{
	...
	if (!list_empty(&bfqd->dispatch)) {
		rq = list_first_entry(&bfqd->dispatch, struct request,
				      queuelist);
		if (q->io_bytes_enable && !rq->ioprio_override &&
		    atomic_long_read(&q->io_bytes) > threshold)
			goto exit;
		list_del_init(&rq->queuelist);
		 ...
		 */
		/*
		 * This path does not go through inc_in_driver_start_rq, so
		 * rq->request_size_bytes is not added to q->io_bytes.
		 * Clear it to avoid unmatched subtraction in
		 * blk_mq_account_free_request().
		 */
		rq->request_size_bytes = 0;
		goto start_rq;
	}

	/*
	 * While throttled, if there are burst requests in the system,
	 * only allow dispatch from bfqd->dispatch (where burst requests
	 * are inserted directly). Do not pick normal requests from trees.
	 */
	if (q->io_bytes_enable && atomic_long_read(&q->io_bytes) > threshold &&
	    atomic_read(&q->burst_ios) > 0)
		goto exit;
...
inc_in_driver_start_rq:
		atomic_long_add(rq->request_size_bytes,
				&q->io_bytes);
...
static unsigned int bfq_calc_request_size_bytes(struct request *rq)
{
	struct bio *bio;
			       ...
			       blk_insert_t flags)
{
	...
	blk_opf_t cmd_flags;
	LIST_HEAD(free);
	/* 根据当前进程的优先级覆盖标志设置请求的优先级覆盖标志 */
	struct task_struct *curr = current;
	rq->ioprio_override = curr->ioprio_override ? true : false;
#ifdef CONFIG_BFQ_GROUP_IOSCHED
	...
	if (q->io_bytes_enable && rq->ioprio_override)
		atomic_inc(&q->burst_ios);
	bfq_update_burst_mode(q);
	rq->request_size_bytes = bfq_calc_request_size_bytes(rq);
...
static void bfq_insert_requests(struct blk_mq_hw_ctx *hctx,
				...
				blk_insert_t flags)
{
	// current宏找到当前进程，读出是否是高优先级进程然后传递给request 标记
 ...
 * the scheduler.
 */
static void __bfq_end_request(struct request *rq, bool is_requeue)
{
	struct bfq_queue *bfqq = RQ_BFQQ(rq);
	struct bfq_data *bfqd;
	unsigned long flags;
	...
	/*
	 * Requeued requests leave the driver without going through
	 * blk_mq_account_free_request(), so drop their in-flight byte
	 * accounting here. Completed requests are accounted in the core path.
	 */
	if (is_requeue && (rq->rq_flags & RQF_STARTED))
		atomic_long_sub((long)rq->request_size_bytes, &rq->q->io_bytes);
...
{
	__bfq_end_request(rq, true);
}

static void bfq_finish_request(struct request *rq)
{
	__bfq_end_request(rq, false);
...
static int bfq_init_queue(struct request_queue *q, struct elevator_type *e)
{
	...
	spin_lock_irq(&q->queue_lock);
	q->elevator = eq;
	atomic_long_set(&q->io_bytes, 0);
	atomic_long_set(&q->io_bytes_threshold, 0);
	atomic_set(&q->burst_ios, 0);
	spin_unlock_irq(&q->queue_lock);
	...
	bfqd->queue->io_bytes_enable = false;
	bfqd->queue->io_high_bytes = 0;
	bfqd->queue->io_low_bytes = 0;
...
SHOW_FUNCTION(bfq_bytes_enable_show, bfqd->queue->io_bytes_enable, 0);
SHOW_FUNCTION(bfq_high_bytes_show, bfqd->queue->io_high_bytes, 0);
SHOW_FUNCTION(bfq_low_bytes_show, bfqd->queue->io_low_bytes, 0);
...
STORE_FUNCTION(bfq_high_bytes_store, &bfqd->queue->io_high_bytes, 0, INT_MAX, 0);
STORE_FUNCTION(bfq_low_bytes_store, &bfqd->queue->io_low_bytes, 0, INT_MAX, 0);
#undef STORE_FUNCTION

static ssize_t
bfq_bytes_enable_store(struct elevator_queue *e, const char *page, size_t count)
{
	struct bfq_data *bfqd = e->elevator_data;
	unsigned long data;
	int ret;

	ret = bfq_var_store(&data, page);
	if (ret)
		return ret;

	bfqd->queue->io_bytes_enable = !!data;
	return count;
}
